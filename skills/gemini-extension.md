# TermiCanvas — Comunicação Entre Agentes (Gemini CLI)

Extensão para o Gemini CLI que ensina o agente a enviar mensagens a outros agentes hospedados no mesmo canvas TermiCanvas (Claude Code, outras instâncias Gemini, etc.).

## Quando usar

Use esta extensão sempre que precisar:
...
- **Dividir trabalho** entre agentes especializados (ex.: pedir ao Claude Code que cuide do back enquanto você cuida do front)
- Encaminhar contexto que outro agente precisa para continuar uma tarefa

Não use para conversas com o próprio usuário — esta extensão é exclusivamente entre agentes.
